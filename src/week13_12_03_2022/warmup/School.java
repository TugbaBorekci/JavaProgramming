...
public class School {
    ...
    private static ArrayList<Teacher> getTeachersByGender(char f) {
        ArrayList<Teacher> result = new ArrayList<>();
        //i need to get teacher one by one
           for (Teacher teacher : teachers) {

       //i need to check the gender if the gender female 'F'
            //if (teacher.gender == gender)//burasi hata veriyor kontrol et
    ...
    private static ArrayList<Teacher> getTeachersNameStatrtWith(String prefix) { //kisayoldan alt+Enter a basarak burayi olusturuyoruz ve yukardaki yer aktiflesiyor hata veermiyor
        ...
        for (Teacher teacher : teachers) {
            ...
            //i need to check the name if the name is starting with"Mi"
